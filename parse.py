...
def open_jsons(task):
    ...
    techmap_path = str(task['techmap']) + "/techmap_result.json"
    synth_path = str(task['synthesis']) + "/synthesis_result.json"
...
def get_bench_stat(attr_name, tech_bench_info, synth_bench_info):
    """
        get stat of a benchmark based on its records
        in both synth and techmap json files
    """
    attr_new_value = {}
    ...
    if (attr_name in tech_bench_info) and (attr_name in synth_bench_info):
        if attr_name not in str_attr_list:
            ...
            diff_percent = - \
                1 if (synth_val == 0) else (
                    diff_val / synth_val) * 100
            attr_new_value = {
...
def compare(techmap, synth):
    ...
    for bt in enumerate(techmap):
        ...
        synth_bench_info = synth[benchmark_name] if (benchmark_name in synth) else ""
        ...
        if (benchmark_name not in synth):
            ...
            for attr in enumerate(tech_bench_info):
                attr_name = attr[1]
                # getting bench statistics
                comparison_record[attr_name] = get_bench_stat(
                                                    attr_name,
                                                    tech_bench_info,
                                                    synth_bench_info
                                                )
    ...
    # iterate over missed benchmarks in synthesis results
    # which are not available in techmap results
    for bt in enumerate(synth):
        comparison_record = {}
        benchmark_name = bt[1]
        if ((benchmark_name not in comparison_results)
            and
                (benchmark_name not in bench_skip_list)):
...
if __name__ == "__main__":
    ...
    for task in enumerate(tasks):
        ...
        output_path = f"./data/{task_name}_compared.json"
        with open(f"{output_path}", "w+") as output:
            serialized_json = json.dumps(to_json, indent=4, sort_keys=False)
            output.truncate(0)
            output.write(serialized_json)
            print(f"Task \"{task_name}\" Comparison files "
                  f"successfully generated in \"{output_path}\"!\n")
